export async function onRequest(context) {
    ...
    // 提取图片名（不带扩展名）作为KV的key
    const imageName = url.pathname.split("/").pop().split(".")[0];
    ...
    let record = await env.img_url.getWithMetadata(imageName);
    if (!record || !record.metadata) {
        ...
        await env.img_url.put(imageName, "", { metadata: record.metadata });
    }
    ...
    const metadata = {
        ...
        fileName: record.metadata.fileName || imageName,
            ...
            if (!moderateResponse.ok) {
                ...
                if (moderateData && moderateData.rating_label) {
                    ...
                    if (moderateData.rating_label === "adult") {
                        ...
                        await env.img_url.put(imageName, "", { metadata });
